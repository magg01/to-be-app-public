...
// Unmounts React trees that were mounted with render and
// clears screen variable that holds latest render output
afterEach(() => {
  cleanup();
});
...
beforeEach(() => {
  render(
    <AddPlan
      toBeId={0}
      onAdd={mockOnNewPlanAdded}
      toBeItemTitle={toBeItemTitle}
...
describe('add button', () => {
  ...
  it('should show the empty-title-alert if there is no text input present when pressed, with the correct message', () => {
    const mockedAlert = jest.spyOn(Alert, 'alert').mockImplementation(() => null);
    const addButton = screen.queryAllByText(CONSTANT_STRINGS.PLANS.ADD_PLAN.ADD_BUTTON)[0];
