...
describe('search input text', () => {
  ...
  it('should hit the api with a new query upon submission of non-blank text', async () => {
    ...
    expect(spiedApiGetPhotos)
      .toHaveBeenCalledWith(expect.objectContaining({ query: newProvidedSearchQuery }));
  });

  it('should not hit the api with a new query upon submission of blank text', async () => {
    render(
      <UnsplashImageSearch
        providedSearchQuery="Query from props"
        onImageDownload={null}
        width="100"
        height="100"
      />,
  ...
  it('should send the lowercase transform of the input to the api', async () => {
    ...
    expect(spiedApiGetPhotos)
      .toHaveBeenCalledWith(expect
        .objectContaining({ query: newProvidedSearchQuery.toLowerCase() }));
  });
...
describe('activity indicator', () => {
  it('should be shown on initial render, given a non-blank providedSearchQuery prop', async () => {
    render(<UnsplashImageSearch providedSearchQuery="test query" />);
  ...
  it('should be hidden once the api call resolves', async () => {
    ...
    render(<UnsplashImageSearch providedSearchQuery="test query" />);
    // get the result object value of the first call to the mocked
    // apiGetPhotos function which should be a promise
    const resultOfApiCall = spiedApiGetPhotos.mock.results[0].value;
    // wrap in act because once the promise resolves the state is updated.
    // Await the fulfillment of act before continuing to prevent unexpected behaviour
    await act(async () => {
      // wait for the result of the api call to be
      // resolved and check the resolved value is the mocked response object
...
describe('on successful response from the api with no results', () => {
  it('should display only the warning message', async () => {
    // update the spiedApiGetPhotos mock to resolve to the
    // unsplash empty response the next time it is called
    ...
    // Await the fulfillment of act before continuing to prevent unexpected behaviour
    await act(async () => {
      // wait for the result of the api call to be resolved and
      // check the resolved value is the mocked response object
...
describe('on error response from api', () => {
  it('should display the first error message from the errors object in the api response', async () => {
    // update the spiedApiGetPhotos mock to resolve to the
    // unsplash error response the next time it is called
    ...
    // Await the fulfillment of act before continuing to prevent unexpected behaviour
    await act(async () => {
      // wait for the result of the api call to be resolved and check the
      // resolved value is the mocked response object
  ...
  it('should display the UNSPLASH_IMAGE_SEARCH_ON_ERROR_RESPONSE_MESSAGE error message', async () => {
    // update the spiedApiGetPhotos mock to resolve to the unsplash
    // error response the next time it is called
    spiedApiGetPhotos.mockResolvedValueOnce(mockApiGetPhotosErrorResponse);
    // render the component (render is wrapped in an act() call so no need to use here)
    render(<UnsplashImageSearch providedSearchQuery="test query" />);
    // get the result object value of the first call to the mocked
    ...
    // Await the fulfillment of act before continuing to prevent unexpected behaviour
    await act(async () => {
      // wait for the result of the api call to be resolved and check
      // the resolved value is the mocked response object
      await expect(resultOfApiCall).resolves.toEqual(mockApiGetPhotosErrorResponse);
    });
    expect(screen.getByText(
      CONSTANT_STRINGS.UNSPLASH_IMAGE_SEARCH.ON_ERROR_RESPONSE_MESSAGE,
